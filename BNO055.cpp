...
#include "Timer.h"
BNO055::BNO055() {
  i2c = DigitalModule::GetInstance(1)->GetI2C(BNO055_ADDRESS<<1);
  i2c->SetCompatibilityMode(false);
  uint8_t buf = 0;
  while (buf!=BNO55_ID)
  {
    i2c->Read(0x00, 1, &buf);
  ...
  while (buf!=0xF)
  {
    ...
    printf("BNO055: Waiting for gyro self-test...\n");
    Wait(0.5);
  }
